...
class Crawler {
  ...
  queueIps (options, ipsPath) {
    this.ipQueues = pushQueue(options)
  ...
  // 启动
  async start (fn = () => {}, ipsPath) {
    let ips = []
    if (ipsPath) {
      ips = require(ipsPath) // 直接传 ip 池地址
    }

    let checked = false // 校验状态
    if (!ips.length && this.ipQueues.length) {
      console.log('开始获取 ip 池:')
      ips = await getIps(this.ipQueues, this.ipsPath) // 如果有自动获取 ip 池，会优先从自动保存的地址获取 ip 池
      checked = true
    }
