...
const globalConfig = ['connections', 'rateLimit']  // 控制器配置
const defaultconfig = {
...
function log (str, obj) {  // 错误日志
  throw new Error(str + JSON.stringify(obj))
}

function separateConfig (config) {  // 分离请求与控制器配置
  let arr = [{}, {}]
...
function pushQueue (options, queues) {  // 请求 push 进队列
...
class Crawler {
  ...
  init (config) {
    ...
    this.globalConfig = configList[0]  // 全局控制器配置
    this.reqConfig = configList[1]  // 全局请求配置
  }

  queueIps (options, path) {  // 添加获取 ip 的请求进入队列
